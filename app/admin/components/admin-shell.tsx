  ...
  return <div className="admin-shell">
    ...
    <div className="admin-workspace">
      <header className="admin-header">
        ...
        <div className="user-menu"><span className="user-avatar" aria-hidden="true">{user.name.slice(0, 1).toUpperCase()}</span><span><strong>{user.name}</strong><small>{user.email}</small></span><Link className="text-button" href="/admin/account/security">Security</Link><form action="/api/auth/sign-out" method="POST"><button className="text-button" type="submit">Sign out</button></form></div>
      </header>

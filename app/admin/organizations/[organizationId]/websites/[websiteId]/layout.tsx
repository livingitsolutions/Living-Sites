...
export default async function WebsiteWorkspaceLayout({ children, params }: { children: ReactNode; params: Promise<{ organizationId: string; websiteId: string }> }) {
  ...
  const result = await composition.getWebsite({ organizationId: context.organization.id, websiteId: websiteId as never }, { authenticatedUser: { userId: context.platformUser.id }, authorizationService: composition.authorizationService, websiteReader: composition.websiteRepository });

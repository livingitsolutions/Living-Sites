...
import { archivePageAction, createPageAction, restorePageAction, updatePageAction } from "./actions";
...
export default async function PagesPage({ params }: { params: Promise<{ organizationId: string; websiteId: string }> }) {
  ...
  const [result, canCreate, canUpdate, canArchive] = await Promise.all([composition.listWebsitePages({ organizationId: context.organization.id, websiteId: websiteId as never }, { authenticatedUser: { userId: context.platformUser.id }, authorizationService: composition.authorizationService, websiteReader: composition.websiteRepository, pageReader: composition.pageRepository }), authorization(PagePermissions.Create), authorization(PagePermissions.Update), authorization(PagePermissions.Archive)]);
  const createAction = createPageAction.bind(null, organizationId, websiteId); const updateAction = updatePageAction.bind(null, organizationId, websiteId); const archiveAction = archivePageAction.bind(null, organizationId, websiteId); const restoreAction = restorePageAction.bind(null, organizationId, websiteId);

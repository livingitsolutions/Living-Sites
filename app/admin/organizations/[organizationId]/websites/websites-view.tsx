...
export function WebsitesView({ websites, canCreate, createAction }: { websites: readonly Website[]; canCreate: boolean; createAction?: (state: CreateWebsiteState, formData: FormData) => Promise<CreateWebsiteState> }) {
  ...
  return <div className="website-list" data-testid="website-list">
    {websites.map((website) => <Card key={website.id} className="website-row">
      ...
      <dl className="website-details"><div><dt>Domain</dt><dd>{website.customDomain ?? website.fallbackDomain}</dd></div><div><dt>Created</dt><dd>{formatDate(website.audit.createdAt)}</dd></div><div><dt>Updated</dt><dd>{formatDate(website.audit.updatedAt)}</dd></div></dl>
    </Card>)}
  </div>;
}

...
export function WebsitesView({ organizationId, websites, canCreate, canPublish, createAction, publishAction, unpublishAction }: { organizationId: string; websites: readonly Website[]; canCreate: boolean; canPublish: boolean; createAction?: (state: CreateWebsiteState, formData: FormData) => Promise<CreateWebsiteState>; publishAction: (formData: FormData) => Promise<void>; unpublishAction: (formData: FormData) => Promise<void> }) {
  ...
  return <div className="website-list" data-testid="website-list">
    {websites.map((website) => <Card key={website.id} className="website-row">
      ...
      <div className="website-row-actions"><dl className="website-details"><div><dt>Domain</dt><dd>{website.customDomain ?? website.fallbackDomain}</dd></div><div><dt>Created</dt><dd>{formatDate(website.audit.createdAt)}</dd></div><div><dt>Updated</dt><dd>{formatDate(website.audit.updatedAt)}</dd></div></dl><div className="page-actions">{canPublish && website.status !== "archived" ? <form action={website.status === "published" ? unpublishAction : publishAction}><input type="hidden" name="websiteId" value={website.id} /><input type="hidden" name="version" value={website.version} /><button className="button button-primary">{website.status === "published" ? "Unpublish Website" : "Publish Website"}</button></form> : null}<Link className="button button-secondary" href={`/admin/organizations/${organizationId}/websites/${website.id}/pages`}>Open workspace</Link></div></div>
    </Card>)}
  </div>;
}

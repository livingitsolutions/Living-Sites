...
const stringProp = (props: RenderableSection["props"], key: string) => typeof props[key] === "string" ? props[key] : "";
const entries = (props: RenderableSection["props"], key: string) => Array.isArray(props[key]) ? props[key] as Record<string, unknown>[] : [];

export function RegisteredSectionRenderer({ section, unknownFallback = null }: { readonly section: RenderableSection; readonly unknownFallback?: React.ReactNode }) {
  const type = getSectionType(String(section.sectionTypeId));
  if (!type) return unknownFallback;
  ...
  switch (type.rendererKey) {
    ...
    default: return unknownFallback;
  }
}

...
import { addOrganizationMember, changeOrganizationMemberRole, removeOrganizationMember, getOrganizationMembers, } from "./index";
...
describe("Membership Use Cases", () => {
    ...
    describe("AddOrganizationMember", () => {
        ...
        it("rejects duplicate active membership", async () => {
            ...
            expect(res.ok).toBe(false);
            if (!res.ok) {
                expect(res.error.code).toBe("duplicate_membership");
            }
        });
    });
    describe("ChangeOrganizationMemberRole", () => {
        ...
        it("prevents demoting the sole remaining owner", async () => {
            ...
            expect(res.ok).toBe(false);
            if (!res.ok) {
                expect(res.error.code).toBe("cannot_demote_sole_owner");
            }
        });
        it("rejects mutation on version mismatch (optimistic concurrency)", async () => {
            ...
            expect(res.ok).toBe(false);
            if (!res.ok) {
                expect("expectedVersion" in res.error || res.error.code === "concurrency_conflict").toBe(true);
            }
        });
    });
    describe("RemoveOrganizationMember", () => {
        ...
        it("prevents removing the sole remaining owner", async () => {
            ...
            expect(res.ok).toBe(false);
            if (!res.ok) {
                expect(res.error.code).toBe("cannot_remove_sole_owner");
            }
        });
    });

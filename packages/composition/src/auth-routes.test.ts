...
import type { TestComposition } from "@livingsites/composition";
...
describe("Auth route behavior", () => {
  ...
  it("registration mode 'open' allows registration", async () => {
    const result = await comp.registerUser(
      { email: "open@test.com", password: "very-secure-password-123", displayName: "Open" },
      comp.registerUserDeps,
    );
  ...
  it("registration mode 'disabled' blocks registration", async () => {
    ...
    const result = await comp.registerUser(
      { email: "disabled@test.com", password: "very-secure-password-123", displayName: "Disabled" },
      comp.registerUserDeps,
    );
  ...
  it("registration mode 'invite_only' blocks without invitation", async () => {
    ...
    const result = await comp.registerUser(
      { email: "invite@test.com", password: "very-secure-password-123", displayName: "Invite" },
      comp.registerUserDeps,
    );
    expect(result.ok).toBe(false);
    if (!result.ok) {
  ...
  it("authenticated /admin succeeds with a valid session", async () => {
    ...
    const regResult = await comp.registerUser(
      { email: "admin@test.com", password: "very-secure-password-123", displayName: "Admin" },
      comp.registerUserDeps,
    );
  ...
  it("sign-out invalidates the session", async () => {
    const regResult = await comp.registerUser(
      { email: "signout@test.com", password: "very-secure-password-123", displayName: "Sign Out" },
      comp.registerUserDeps,
    );
  ...
  it("session revocation works", async () => {
    const regResult = await comp.registerUser(
      { email: "revoke@test.com", password: "very-secure-password-123", displayName: "Revoke" },
      comp.registerUserDeps,
    );
    if (!regResult.ok) throw new Error("registration failed");
  ...
  it("password is hashed, not stored in plaintext", async () => {
    ...
    const result = await comp.registerUser(
      { email: "safe@test.com", password: "very-secure-password-123", displayName: "Safe" },
      comp.registerUserDeps,
    );
    expect(result.ok).toBe(true);

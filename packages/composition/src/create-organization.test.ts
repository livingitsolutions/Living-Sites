...
function makePlan(overrides: Partial<Plan> = {}): Plan {
  return {
    ...
    isActive: true,
    version: 1,
    audit: { createdAt: "2026-01-01T00:00:00.000Z" as ISODateString, updatedAt: "2026-01-01T00:00:00.000Z" as ISODateString },
    ...overrides,
  } as Plan;
}

describe("createOrganization use case (via composeTest)", () => {
  let comp: ReturnType<typeof composeTest>;
  ...
  it("returns version 1 after creation", async () => {
    const result = await comp.createOrganization(
      { name: "Acme Corp", slug: "acme-corp", billingEmail: "billing@acme.com" },
      comp.createOrganizationDeps,
    );
    expect(result.ok).toBe(true);
    if (result.ok) {
  ...
  it("fails for empty name", async () => {
    const result = await comp.createOrganization(
      { name: "", slug: "acme-corp", billingEmail: "billing@acme.com" },
      comp.createOrganizationDeps,
    );
  ...
  it("fails for invalid slug", async () => {
    const result = await comp.createOrganization(
      { name: "Acme", slug: "INVALID SLUG!", billingEmail: "billing@acme.com" },
      comp.createOrganizationDeps,
    );
    expect(result.ok).toBe(false);
    if (!result.ok) {
  ...
  it("fails for invalid email", async () => {
    const result = await comp.createOrganization(
      { name: "Acme", slug: "acme-corp", billingEmail: "not-email" },
      comp.createOrganizationDeps,
    );
    expect(result.ok).toBe(false);
    if (!result.ok) {
  ...
  it("fails for duplicate slug", async () => {
    await comp.createOrganization(
      { name: "First Org", slug: "dup-slug", billingEmail: "a@a.com" },
      comp.createOrganizationDeps,
    );
    const result = await comp.createOrganization(
      { name: "Second Org", slug: "dup-slug", billingEmail: "b@b.com" },
      comp.createOrganizationDeps,
    );
    expect(result.ok).toBe(false);
    if (!result.ok) {
  ...
  it("fails for inactive plan", async () => {
    ...
    const result = await comp.createOrganization(
      { name: "Acme", slug: "acme-corp", billingEmail: "billing@acme.com", planId: plan.id },
      comp.createOrganizationDeps,
    );
    expect(result.ok).toBe(false);
    if (!result.ok) {
  ...
  it("fails for non-existent plan", async () => {
    const result = await comp.createOrganization(
      { name: "Acme", slug: "acme-corp", billingEmail: "billing@acme.com", planId: "plan_nonexistent" as PlanId },
      comp.createOrganizationDeps,
    );
    expect(result.ok).toBe(false);
    if (!result.ok) {
  ...
  it("denies reserved slug", async () => {
    const result = await comp.createOrganization(
      { name: "Admin Org", slug: "admin", billingEmail: "admin@test.com" },
      comp.createOrganizationDeps,
    );
    expect(result.ok).toBe(false);
    if (!result.ok) {
  ...
  it("emits OrganizationCreated event on success", async () => {
    const result = await comp.createOrganization(
      { name: "Event Org", slug: "event-org", billingEmail: "event@test.com" },
      comp.createOrganizationDeps,
    );
    expect(result.ok).toBe(true);
  ...
  it("emits no event on validation failure", async () => {
    await comp.createOrganization(
      { name: "", slug: "event-org", billingEmail: "event@test.com" },
      comp.createOrganizationDeps,
    );
  ...
  it("emits no event on policy denial", async () => {
    await comp.createOrganization(
      { name: "Admin", slug: "admin", billingEmail: "a@a.com" },
      comp.createOrganizationDeps,
    );
    expect(comp.eventPublisher.count).toBe(0);
  });
  ...
  it("emits no event on duplicate slug", async () => {
    await comp.createOrganization(
      { name: "First", slug: "dup", billingEmail: "a@a.com" },
      comp.createOrganizationDeps,
    );
    comp.eventPublisher.clear();
    await comp.createOrganization(
      { name: "Second", slug: "dup", billingEmail: "b@b.com" },
      comp.createOrganizationDeps,
    );
    expect(comp.eventPublisher.count).toBe(0);
  });
  ...
  it("uses injected Clock for timestamps", async () => {
    ...
    const result = await comp.createOrganization(
      { name: "Clock Test", slug: "clock-test", billingEmail: "c@c.com" },
      comp.createOrganizationDeps,
    );
    expect(result.ok).toBe(true);
    if (result.ok) {
  ...
  it("uses injected IdGenerator for ID generation", async () => {
    const result = await comp.createOrganization(
      { name: "ID Test", slug: "id-test", billingEmail: "i@i.com" },
      comp.createOrganizationDeps,
    );
    expect(result.ok).toBe(true);
    if (result.ok) {

...
export function rowToPage(row, sectionRows = []) {
    ...
    return { ok: true, value: { id: row.id, websiteId: row.website_id, title: row.title, slug: row.slug,
            ...
            publishedSnapshotId: row.published_snapshot_id, sectionOrder: row.section_order, sections: sectionRows.map(rowToSection), availableLocales: row.available_locales,
...
function rowToSection(row) {
    return { id: row.id, pageId: row.page_id, websiteId: row.website_id, sectionTypeId: row.section_type_id, props: row.props, sortOrder: row.sort_order, status: row.status, audit: { createdAt: row.created_at.toISOString(), updatedAt: row.updated_at.toISOString(), ...(row.created_by ? { createdBy: row.created_by } : {}), ...(row.updated_by ? { updatedBy: row.updated_by } : {}) } };
}

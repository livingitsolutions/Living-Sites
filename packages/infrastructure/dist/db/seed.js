...
export async function seedPlansAndFeatures(db, logger) {
    let plansUpserted = 0;
    let featuresUpserted = 0;
    let entitlementsUpserted = 0;
    for (const plan of PLAN_SEEDS) {
        const allPlans = await db.select().from(plans);
        const found = allPlans.find((p) => p.id === plan.id);
        if (found) {
    ...
    for (const feature of FEATURE_SEEDS) {
        ...
        const found = allFeatures.find((f) => f.id === feature.id);
        if (found) {
    ...
    for (const ent of ENTITLEMENT_SEEDS) {
        ...
        const found = allEnts.find((e) => e.id === ent.id);
        if (found) {

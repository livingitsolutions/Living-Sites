...
describeOrSkip("Drizzle Plan/Feature/Outbox — database integration", () => {
    ...
    describe("PlanReader", () => {
        it("returns active plan by ID", async () => {
            const found = await planReader.findById(PLAN_FREE_ID);
        ...
        it("returns null for missing plan", async () => {
            const found = await planReader.findById("plan_nonexistent");
        ...
        it("reconstructs plan with entitlements", async () => {
            const found = await planReader.findById(PLAN_LIFETIME_ID);
            expect(found).not.toBeNull();
        ...
        it("returned values are isolated", async () => {
            ...
            found.name = "Modified";
            const refound = await planReader.findById(PLAN_FREE_ID);
    ...
    describe("OrganizationCreationPersistence — atomic transaction", () => {
        function makeDraft(overrides) {
            return createOrganizationDraft({
                id: (overrides.id ?? `org_tx_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`),
                name: overrides.name ?? "Tx Test Org",
                slug: (overrides.slug ?? `tx-test-${Date.now()}`),
...
function makeEventHelper(draftId, slug) {
    return {
        type: "organization.created",
        occurredAt: "2026-01-01T00:00:00.000Z",
        organizationId: draftId,

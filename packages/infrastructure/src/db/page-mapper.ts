import type { AggregateVersion, ISODateString, LocaleCode, Page, PageDraft, PageId, SectionId, Slug, UserId, WebsiteId } from "@livingsites/domain";
...
export function rowToPage(row: PageRow): MapperResult {
  ...
  return { ok: true, value: { id: row.id as PageId, websiteId: row.website_id as WebsiteId, title: row.title, slug: row.slug as Slug,
    ...
    publishedSnapshotId: row.published_snapshot_id, sectionOrder: row.section_order as SectionId[], availableLocales: row.available_locales as LocaleCode[],

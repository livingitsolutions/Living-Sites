...
describe("DrizzlePagePublicationRepository — Netlify Database", () => {
  ...
  it.each(["snapshot", "event"] as const)("rolls back everything when %s persistence fails", async (failure) => { const repository = new DrizzlePagePublicationRepository({ db, logger: new NoopLogger(), ...(failure === "snapshot" ? { beforeSnapshotInsert: () => { throw new Error("snapshot failure"); } } : { beforeOutboxInsert: () => { throw new Error("event failure"); } }) }); expect((await repository.publish({ candidate: candidate("snapshot_fail", "First"), expectedPageVersion: 1, event: event("snapshot_fail") })).ok).toBe(false); expect(await db.select().from(pageSnapshots)).toHaveLength(0); expect((await db.select().from(pages))[0]).toMatchObject({ status: "draft", published_snapshot_id: null, version: 1 }); expect(await db.select().from(applicationOutbox)).toHaveLength(0); });
  it("publishes nothing for a stale Page version", async () => { const repository = new DrizzlePagePublicationRepository({ db, logger: new NoopLogger() }); expect((await repository.publish({ candidate: candidate("snapshot_stale", "First"), expectedPageVersion: 9, event: event("snapshot_stale") })).ok).toBe(false); expect(await db.select().from(pageSnapshots)).toHaveLength(0); });
  it("prevents concurrent inserts from creating duplicate revision numbers", async () => {
    const values = (id: string) => ({ id, page_id: pageId, website_id: websiteId, organization_id: orgId, revision_number: 1, page_metadata: candidate(id, id).page, sections: candidate(id, id).sections, created_at: new Date(now), published_at: new Date(now), published_by: userId });

...
function makeDraft(overrides: Partial<{
  ...
  email: string;
  displayName: string;
}>): import("@livingsites/domain").UserDraft {
  return createUserDraft({
    id: (overrides.id ?? "user_test_001") as UserId,
    authSubjectId: (overrides.authSubjectId ?? "auth_test_001") as AuthSubjectId,
...
export function runUserRepositoryContractTests(
  name: string,
  getFixtures: () => UserRepositoryContractFixtures,
) {
  describe(`${name} — user repository contract`, () => {
    let f: UserRepositoryContractFixtures;
    ...
    it("findByAuthSubjectId reconstructs the aggregate after create", async () => {
      ...
      const found = await f.reader.findByAuthSubjectId("auth_subj_001" as AuthSubjectId);
      expect(found).not.toBeNull();
    ...
    it("returns null for non-existent id", async () => {
      const found = await f.reader.findById("nonexistent" as UserId);
      expect(found).toBeNull();
    });

    it("returns null for non-existent authSubjectId", async () => {
      const found = await f.reader.findByAuthSubjectId("nonexistent" as AuthSubjectId);
      expect(found).toBeNull();
    });
    ...
    it("no database/Drizzle row type leaks to Application", () => {
      const user: User | null = null as User | null;
